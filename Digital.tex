...
\usepackage[absolute,overlay]{textpos} % falls du textuelle Positionierung bevorzugst
...
% no header, no footer.
\pagestyle{empty}
\cleardoublepage         % Neue rechte Seite (zwingt Seite mit gerader/ungerader Logik)
% Deckblatt mit Copyright
\includepdf[
  pages=1,
  noautoscale=true,
  offset=0mm 0mm,
  ...
  pagecommand={
    \begin{tikzpicture}[remember picture, overlay]
      \node[anchor=south east, xshift=-1cm, yshift=1cm] at (current page.south east) 
        {\textcolor{red}{© 2025 FS Mathematik Gymnasium Gröbenzell}};
    \end{tikzpicture}
  }
]{Deckblatt1.pdf}
\cleardoublepage         % Inhalt beginnt auf nächster rechten Seite
\pagestyle{empty}
\section{Analysis}
...
\begin{itemize}
    \item Eine Funktion heißt \textcolor{red}{umkehrbar}, wenn verschiedene Werte des Definitionsbereichs $\mathds{D}$ verschiedene Werte des Wertebereichs $\mathds{W}$ haben.
    \item Diese eindeutig umgekehrte Zuordnung nennt man \textcolor{red}{Umkehrfunktion} $f^{-1}$.
    \item Kriterium für die Umkehrbarkeit: Ist eine Funktion $f$ in einem Intervall $I \subseteq \mathds{R}$ streng monoton, dann ist $f$ in $I$ umkehrbar.
    \item Eigenschaften der Umkehrfunktion $f^{-1}$\\
    \begin{enumerate}
    \item $\mathds{D}_{f^{-1}} = \mathds{W}_f$ \item $\mathds{W}_{f^{-1}} = \mathds{D}_f$
\end{enumerate}
\item Bildung der Umkehrfunktion $f^{-1}$:\\
\begin{enumerate}
    \item Spiegelung des Graphen $G_f$ an der Geraden mit der Gleichung $y=x$\\
    \item Variablentausch von $x$ und $y$ und anschließendes auflösen nach $y$ 
\end{enumerate}
\end{itemize}
...
\begin{tikzpicture}[scale=0.25]
  ...
  \draw[thick,domain=-2:2,smooth,variable=\x,red] 
    plot ({\x},{\x*\x});
\end{tikzpicture}
\item a ungerade: \begin{tikzpicture}[scale=0.2]
  % Achsen
...
\end{tabular} 
    ...
    \item Graph:\\ \scalebox{0.6}{\begin{tikzpicture}
...
\tikzstyle{bag} = [circle, draw, text width=1em, inner sep=2pt, text centered]
\tikzstyle{end} = [circle, minimum width=5pt, fill, inner sep=0pt]
\scalebox{0.55}{\begin{tikzpicture}[grow=down, sibling distance=4cm, level distance=2.5cm]
  \tikzset{bag/.style={circle, draw=black, minimum size=6mm, inner sep=2pt}}
...
\underbrace{(1 - p)^{n - k}}_{\shortstack{\footnotesize{\textcolor{blue}{Nieten-}}\\ \footnotesize{\textcolor{blue}{wahrscheinlichkeit}}}}$\\
    \item Mindestens ein Treffer\\ $P_p^n(X\geq 1) = 1- P_p^n(X=0) = 1- \binom{n}{0}\cdot p^0 \cdot q^n = 1-q^n$\\
...
\scalebox{0.8}{\begin{tabular}{|>{\columncolor{gray!15}}l|>{\columncolor{gray!10}}c|>{\columncolor{gray!10}}c|}
  \hline
  \rowcolor{gray!20}
