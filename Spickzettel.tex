...
\fill[blue!20] (-1.5,2.9) rectangle (5.5,3.1);
\draw[->] (-2.2,3) -- (7,3) node[right] {$x$};
\foreach \x in {-2,-1,0,1,2,3,4,5,6}
  \draw (\x,2.9) -- (\x,3.1) node[below=2pt] {\small \x};
\node[blue] at (5.2,3.4) {Wertemenge $W$};
...
Dann gilt für die Wahrscheinlichkeitsverteilung dieser Zufallsgröße $P(X = k) = \frac{\binom{S}{k}\cdot \binom{N-S}{n-k}}{\binom{N}{n}}$ mit $k\leq n$ und $k\leq S$
\end{itemize}
\end{document}