...
\mynote $f(x) =\underbrace{x+1}_{\text{Gleichung der \textcolor{red}{schrägen} Asymptote}} +\dfrac{1}{x^2}$\\
...
\end{enumerate}
\item Der Verbindungsvektor $\vv{PQ}$ wird durch $\vv{PQ} = \vv{Q} - \vv{P}$ bestimmt.\\ \mynote "Spitze minus Hacke"
\item Der Mittelpunkt $M$ einer Strecke $AB$ wird durch \\$\vv{M} = \frac{1}{2}\cdot \left(\vv{A} + \vv{B}\right)$ bestimmt 
\end{itemize}
...
\end{tikzpicture} }
\begin{itemize}
\item Drücke den Vektor $\vv{MN}$ mithilfe der Vektoren $\vv{a}, \vv{b}$ und $\vv{c}$ aus.
     \begin{equation*}
            \begin{split}
...
\end{itemize}
%\unsure[inline]{Definition von Vektoren!} 
\end{document}22