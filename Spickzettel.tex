...
\item Vektorprodukt $\longrightarrow$  $$\vv{c} = \vv{a} \times \vv{b} = \begin{pmatrix} a_1 \\a_2\\a_3\end{pmatrix} \times \begin{pmatrix} b_1 \\b_2\\b_3\end{pmatrix} = \begin{pmatrix} a_2\cdot b_3 - a_3\cdot b_2 \\a_3\cdot b_1 - a_1\cdot b_3\\a_1\cdot b_2 - a_2\cdot b_1\end{pmatrix}$$
...
\begin{tikzpicture}
 \begin{scope}[
  ...
  \draw[->,blue, thick] (A) -- (B) node[sloped,midway, below] {$\vec{AB}$};
  \draw[->,blue, thick] (A) -- (C) node[sloped,very near end, below] {$\vec{AC}$};
  \draw[->,blue, thick] (A) -- (S) node[sloped,midway, above] {$\vec{AS}$};

  % Punkte beschriften
  \draw[red] (A) node[below left] {A};
  \draw[red] (B) node[below right] {B};
  \draw[red] (C) node[left] {C};
...
\end{enumerate}
\end{itemize}
\unsure[inline]{Bild eines Spats; Eigenschaften Spatprodukt} 
\end{document}